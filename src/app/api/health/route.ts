...
export async function GET(){try{const owner=await requireOwner();const checkedAt=new Date().toISOString();const services:HealthService[]=[];let databaseOk=true;
  ...
  if(databaseOk){const result=await getPool().query("SELECT service,last_seen FROM service_heartbeats WHERE service=ANY($1::text[])",[["scoring-worker","evaluation-worker"]]);const seen=new Map<string,string>(result.rows.map(row=>[row.service,new Date(row.last_seen).toISOString()]));for(const [id,name] of [["scoring-worker","后台评分"],["evaluation-worker","模型评测"]] as const){const state=heartbeatState(seen.get(id));services.push(service(id,name,state.state,state.message,checkedAt,state.recovery));}}
  else{for(const [id,name] of [["scoring-worker","后台评分"],["evaluation-worker","模型评测"]] as const)services.push(service(id,name,"warning","数据库不可用，暂时无法判断后台服务状态。",checkedAt,"先恢复数据库后再刷新。"));}
