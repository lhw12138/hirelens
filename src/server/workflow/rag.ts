...
import {keywordRank, fuseAndRerank, validVectors, type VectorSource, type RagTrace} from '@/lib/hybrid-rag';
...
async function index(taskId:string,personId:string,sources:Source[],modelKey:string){
 const db=getPool();const snapshot=snapshotFor(sources);const scope=[taskId,personId,snapshot,modelKey];
 const cached=await db.query('SELECT source_id, embedding::text AS embedding FROM workflow_vectors WHERE task_id=$1 AND candidate_id=$2 AND snapshot=$3 AND model_key=$4',scope);
 ...
 const conn=await db.connect();try{await conn.query('BEGIN');for(const s of indexed)await conn.query('INSERT INTO workflow_vectors(task_id,candidate_id,snapshot,model_key,source_id,kind,embedding) VALUES($1,$2,$3,$4,$5,$6,$7::vector) ON CONFLICT DO NOTHING',[...scope,s.id,s.kind,JSON.stringify(s.vector)]);await conn.query('COMMIT');}catch(e){await conn.query('ROLLBACK');throw e;}finally{conn.release();}
 return{snapshot,cacheHit:false,sources:indexed};
}
export async function prepareRetrieval(task:HiringTask,person:Person,criteria:Criterion[],phase:'screening'|'combined'){
 ...
 try{
  ...
  if(trace.mode==='hybrid'){
   ...
   trace.modelKey=health.modelKey;indexed=await index(task.id,person.id,sources,health.modelKey);trace.cacheHit=indexed.cacheHit;trace.indexedChunks=sources.length;
  ...
  for(const [i,c]of criteria.entries()){
   ...
   for(const kind of (phase==='combined'?['resume','answer']:['resume'])){
    ...
    if(indexed){
     const rows=await getPool().query('SELECT source_id FROM workflow_vectors WHERE task_id=$1 AND candidate_id=$2 AND snapshot=$3 AND model_key=$4 AND kind=$5 ORDER BY embedding <=> $6::vector, source_id LIMIT 12',[task.id,person.id,indexed.snapshot,trace.modelKey,kind,JSON.stringify(queries[i])]);
     const vector=rows.rows.map(r=>r.source_id as string);vectorIds.push(...vector);
